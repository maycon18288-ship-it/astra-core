...
import astra.core.player.cash.CashException;
    ...
    try {
      switch (action.toLowerCase()) {
        case "set":
          ...
          sender.sendMessage("§aVocê setou o Cash do(a) " + Rank.getColored(args[1]) + " §apara §b" + StringUtils.formatNumber(amount) + "§a.");
          break;
        case "add":
          CashManager.addCash(args[1], amount);
          sender.sendMessage("§aVocê adicionou §b" + StringUtils.formatNumber(amount) + " §ade Cash para o(a) " + Rank.getColored(args[1]) + "§a.");
          break;
        case "remove":
          CashManager.removeCash(args[1], amount);
          sender.sendMessage("§aVocê removeu §b" + StringUtils.formatNumber(amount) + " §ade Cash do(a) " + Rank.getColored(args[1]) + "§a.");
      }
