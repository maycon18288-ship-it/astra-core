...
import astra.core.api.profile.api.MineToolsAPI;
import astra.core.api.profile.api.MojangAPI;
...
public abstract class Mojang {
  ...
  public static String getSkinProperty(String id) throws InvalidMojangException {
    ...
    String redisKey = "skin:" + id;
    String redisCached = RedisManager.getInstance().get(redisKey);
    if (redisCached != null) {
      return redisCached;
    }
    ...
    for (Mojang api : MOJANGAPIS) {
      ...
      if (api.getResponse()) {
        if (property != null) {
          ...
          // Salva no Redis para cache futuro
          RedisManager.getInstance().set(redisKey, property);
        }
        return property;
      }
    }
