...
public class Hotbar {
  ...
  @Getter private final List<HotbarButton> buttons;
  ...
  public String getName() {
    ...
    public void apply(Account profile) {
    ...
    this.buttons.stream().filter(button -> button.getSlot() >= 0 && button.getSlot() <= 8).forEach(button -> {
      ItemStack icon = BukkitUtils.deserializeItemStack(button.getIcon().replace("%perfil%", ""));
