package astra.core.player.fake;
...
public class FakeManager {
  ...
  private static final AuroraConfig CONFIG = Core.getInstance().getConfig("utils");
  private static final Pattern REAL_PATTERN = Pattern.compile("(?i)acore-corefakereal:\\w*"), NOT_CHANGE_PATTERN = Pattern.compile("(?i)acore-corechange:\\w*");

  public static Map<String, String> fakeNames = new HashMap<>();
  public static Map<String, Rank> fakeRoles = new HashMap<>();
  ...
  public static void setupFake() {
    ...
    for (String roleName : CONFIG.getStringList("fake.role")) {
      Rank role = Rank.getRoleByName(roleName);
      if (role != null) {
  ...
  public static boolean isBungeeSide() {
    if (bungeeSide == null) {
      bungeeSide = (Boolean) CONFIG.getBoolean("bungeecord");
    }
  ...
  public static String replaceNickedChanges(String original) {
    ...
    for (String name : listNicked()) {
      ...
      while (matcher.find()) {
        replaced = replaced.replaceFirst(Pattern.quote(matcher.group()), Matcher.quoteReplacement("acore-corechange:" + name));
      }
    }
  ...
  public static String replaceNickedPlayers(String original, boolean toFake) {
    ...
    for (String name : listNicked()) {
      ...
      while (matcher.find()) {
        ...
        backup.add(found.replace("acore-corechange:", ""));
        replaced = replaced.replaceFirst(Pattern.quote(found), Matcher.quoteReplacement("acore-corechange:" + (backup.size() - 1)));
      }
    ...
    while (matcher.find()) {
      ...
      replaced = replaced.replaceFirst(Pattern.quote(found), Matcher.quoteReplacement(
              fakeNames.entrySet().stream().filter(entry -> entry.getValue().equals(found.replace("acore-corefakereal:", ""))).map(Map.Entry::getKey).findFirst().orElse("")));
    }

    matcher = NOT_CHANGE_PATTERN.matcher(replaced);
    while (matcher.find()) {
      String found = matcher.group();
      replaced = replaced.replaceFirst(Pattern.quote(matcher.group()), Matcher.quoteReplacement(backup.get(Integer.parseInt(found.replace("acore-corechange:", "")))));
    }
