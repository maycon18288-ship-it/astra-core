...
public class AuroraConfig {
  ...
  private AuroraConfig(AuroraPlugin plugin, String path, String name) {
    this.plugin = plugin;
  ...
  private static Map<String, AuroraConfig> cache = new HashMap<>();

  public static AuroraConfig getConfig(AuroraPlugin plugin, String path, String name) {
    if (!cache.containsKey(path + "/" + name)) {
      cache.put(path + "/" + name, new AuroraConfig(plugin, path, name));
    }
