...
public class BungeeRedisManager {
    ...
    @Getter
    private static BungeeRedisManager instance;
    ...
    public BungeeRedisManager(Configuration config, Logger logger) {
        instance = this;
        this.logger = logger;
        ...
        Bungee.getInstance().getLogger().info("Iniciando conexão com Redis em " + host + ":" + port);
        if (!password.isEmpty()) {
        }
        ...
        try {
            ...
            try (Jedis jedis = pool.getResource()) {
                ...
                if ("PONG".equalsIgnoreCase(pong)) {
                    Bungee.getInstance().getLogger().info("Conexão Redis estabelecida com sucesso!");
                }
            }
